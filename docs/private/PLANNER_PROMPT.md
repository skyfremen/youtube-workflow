...
4. Background Management may use repository code and GitHub-held provider credentials for deterministic provider discovery and readiness persistence. It must not perform ChatGPT-owned visual/editorial approval or private video processing. Exact-source frame/motion extraction belongs to the stateless public Review Evidence runtime.
...
6. Consume the matching discovery result only; never fabricate provider IDs, durations, renditions or preview URLs. Once the matching immutable discovery result exists, provider discovery is complete. Review exact-source evidence using the strongest trustworthy visual surface actually available in the current ChatGPT/Work execution environment. Evidence retrieval is **tool-adaptive** and must not depend on any single transport:
   - Prefer native web/browser/image-capable tools against the exact `preview_image_url`, `preview_video_url`, or Pexels `source_page` from the immutable discovery result when those tools are available.
   - If direct URL rendering is unavailable but web/image search is available, query the exact Pexels `provider_asset_id` and/or exact source-page identity and approve evidence only when it can be tied unambiguously to that same provider asset. Never approve a visually similar substitute or a different Pexels asset.
   - Background Management normally dispatches `skyfremen/production-runtime/.github/workflows/review-evidence.yml` for the exact immutable discovery result. Find the matching public run using the immutable discovery `request_id`; its run name is `Review evidence <request_id>` and its artifact is `background-review-evidence-<request_id>`. The public runtime reads the exact private `source_sha`/result path with read-only credentials and produces transport-only representative frames/contact sheets/motion samples. It never approves candidates or writes private state.
   - If the matching public Review Evidence run is queued/in progress, poll its status and artifact for up to the shared 10-minute continuation window rather than returning immediately. If it succeeds, download the artifact through the available GitHub connector/API and inspect the actual frames/pixels/motion. If it reaches terminal failure, report the exact infrastructure/evidence-generation failure and, when safe, regenerate by dispatching private Background Management with the same immutable `discovery_result` path; do not create a new discovery request. If it is still legitimately pending at the bounded deadline, `DEFERRED_REPLENISHMENT` is valid and the next invocation must resume the same request ID/run/artifact path.
   - If no matching public evidence run/artifact exists or an old artifact expired, dispatch private Background Management with the exact existing `content/background-sourcing/discovery-results/<request_id>.json` path. This is a transport regeneration operation only: it must re-dispatch public Review Evidence without repeating provider discovery or changing immutable provider state.
   - If the local runtime actually supports outbound HTTPS/media tooling, `youtube-shorts-bot/media/preview_review_materializer.py` may be materialized at the same exact rules/source SHA and used as an **optional fallback**, not as a required dependency:
...
```
   ...
   - Failure of one evidence transport must cause the planner to try the next available exact-source transport rather than terminate the replenishment attempt. Lack of outbound networking or FFmpeg in local ChatGPT Python is not itself a replenishment blocker when the public evidence artifact or another exact-source visual surface is available.

   Inspect actual pixels for every proposed source. If a still image is genuinely insufficient to judge a candidate and an exact-source video/representative-frame surface is available, inspect representative frames or short playback; full-length/end-to-end playback is unnecessary. A successful fetch/extraction is **not** approval: ChatGPT/Work must still make the semantic/visual decision. Metadata-only approval remains prohibited. Reject candidates whose exact-source evidence remains inaccessible or is visually unsuitable and continue through the remaining discovery set; one inaccessible preview URL or one failed public extraction must not fail the whole replenishment attempt. If too few candidates survive, continue with the next immutable discovery attempt under the same readiness deficits rather than weakening review.

   Only when **all available exact-source visual channels** are genuinely unusable for the discovery set, and no matching public Review Evidence run remains legitimately pending, may the planner stop before a readiness manifest. Report this as `EVIDENCE_ACCESS_BLOCKED`, include the attempted evidence channels, matching public run/artifact evidence if any, and affected discovery-result path, and do not mislabel it `DEFERRED_REPLENISHMENT`.
...
`DEFERRED_REPLENISHMENT` is valid only when a required matching Background Management or public Review Evidence workflow is still legitimately queued/in-progress after its bounded wait. It is **not** valid merely because visual review remains, because local Python lacks internet/media tooling, because a browser/connector cannot render one provider URL, or because one candidate's preview/extraction failed. A `DEFERRED_REPLENISHMENT` report must include profile, plan date, rules/source SHA used before drift refresh, discovery request ID/path, matching private/public workflow run ID/status as applicable, matching evidence artifact/discovery-result/readiness-manifest paths if present, current readiness deficits, and the exact continuation point. It must explicitly say that no ranked pool/request was created. This state is resumable and must never be described as a creative/content failure.

Do not bypass provider discovery by guessing metadata from public pages. Do not move creative/editorial review into GitHub Actions. Do not create a second discovery request simply because an existing matching private/public workflow is still running. Do not make local Python outbound HTTP or FFmpeg a prerequisite for visual review: exhaust the available exact-source web/browser/image/public-artifact/local-evidence surfaces according to step 6, reject candidate-specific evidence failures individually, and use `EVIDENCE_ACCESS_BLOCKED` only when no trustworthy exact-source visual channel remains usable.
...
Git is a source-acquisition mechanism, not the planner. ChatGPT/Work owns creative planning; local canonical Python proves the frozen plan is valid; private GitHub Actions owns deterministic control-plane/provider/state work; the public stateless runtime owns media execution/evidence extraction; neither GitHub side owns creative/editorial approval.
