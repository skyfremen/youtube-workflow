...
for (const [i, scene] of plan.scenes.entries()) {
  ...
  if (!Array.isArray(scene.beats) || scene.beats.length < 3) throw new Error(`Scene ${scene.id || i + 1} requires at least 3 visual beats.`);
...
console.log(`Storyboard valid: ${plan.scenes.length} scenes, ${beats} beats, ${total}s, ${plan.fps}fps, assets=${[...assetCategories].join(',')}.`);
