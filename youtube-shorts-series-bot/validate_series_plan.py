...
def main():
    ...
    for idx, item in enumerate(items, 1):
        ...
        if part_number < part_total:
            ...
            elif next_slot <= item.get('slot', 0):
                errors.append(f'item {idx}: next_part_slot must be later than the current slot')
            elif next_slot - item.get('slot', 0) < 4:
                errors.append(f'item {idx}: next series part should be at least 4 slots later')
        else:
    ...
    if not MIN_SERIES_PARTS_TOTAL <= series_parts_total <= MAX_SERIES_PARTS_TOTAL:
        errors.append(
            f'series parts total must be {MIN_SERIES_PARTS_TOTAL}-{MAX_SERIES_PARTS_TOTAL}; found {series_parts_total}'
        )
    ...
    for sid, parts in series.items():
        ...
        for i in range(len(parts) - 1):
            ...
            if content.get('next_part_slot') != expected_next_slot:
                errors.append(
                    f'series {sid} part {parts[i][0]}: next_part_slot must be {expected_next_slot}'
                )
