...
class Repository:
    ...
    def added_commit(self, path):
        commits = self.git(
            "log", "--diff-filter=A", "--format=%H", "--", self.relative(path)
        ).splitlines()
    ...
    def receipt_state(self, request_path, source_sha, publication_mode):
        ...
        try:
            ...
            valid = (
                ...
                and receipt.get("request_path")
                == f"youtube-shorts-bot/content/requests/{request_path.name}"
    ...
    def batches_for(self, content_id, source_sha):
        ...
        batches = [
            {
                "batch_id": normal_batch_id(source_sha),
                "started_at": normal_started,
                "automatic": False,
                "automatic_attempt": 0,
            }
        ]
            ...
            batches.append(
                {
                    ...
                    "automatic": payload.get("recovery_control", {}).get("kind")
                    == "automatic",
    ...
    def batch_event(self, batch):
        ...
        if completion.exists():
            events.append(
                {
                    "kind": "completion",
                    "at": self.path_time(completion, batch["started_at"]),
                    "retryable": None,
                    "error_code": None,
                    "stage": None,
                }
        ...
        if diag_dir.exists():
            for path in diag_dir.glob("*.json"):
                ...
                events.append(
                    {
                        "kind": "diagnostic",
                        "at": self.path_time(path, batch["started_at"]),
    ...
    def snapshot(self, request_path, *, now, failed_source_sha=""):
        ...
        attempts = max(
            [x["automatic_attempt"] for x in batches if x["automatic"]] or [0]
        )
        return Snapshot(
            ...
            receipt_state=self.receipt_state(
                request_path, source_sha, publication_mode
            ),
    ...
    def write_batch(self, recoverable, *, now, policy):
        items = [
            {
                "content_id": s.content_id,
                "source_commit_sha": s.source_commit_sha,
                "automatic_attempt": s.automatic_attempts + 1,
                "reason": d.reason,
            }
        ...
        if path.exists():
            ...
            old = [
                (
                    x.get("content_id"),
                    x.get("source_commit_sha"),
                    x.get("automatic_attempt"),
                )
                for x in existing.get("items", [])
            ]
            new = [
                (x["content_id"], x["source_commit_sha"], x["automatic_attempt"])
                for x in items
            ]
...
def reconcile(repo, *, now, policy, write, failed_source_sha=""):
    ...
    for path in sorted(repo.requests.glob("*.json")):
        try:
            snap = repo.snapshot(
                path, now=now, failed_source_sha=failed_source_sha
            )
        ...
        decisions.append(
            {
                ...
                "reason": decision.reason,
                "automatic_attempts": snap.automatic_attempts,
    ...
    if write:
        for snap, decision in terminal:
            written.append(
                str(repo.write_terminal(snap, decision, now=now, policy=policy))
            )
        if recoverable:
            batch_id, path = repo.write_batch(
                recoverable, now=now, policy=policy
            )
...
def policy_from_env():
    backoff = tuple(
        int(x)
        for x in os.getenv("RECOVERY_BACKOFF_MINUTES", "0,120,240").split(",")
        if x.strip()
    )
    policy = Policy(
        int(os.getenv("RECOVERY_ACTIVE_GRACE_MINUTES", "210")),
        int(os.getenv("RECOVERY_SCHEDULE_BUFFER_MINUTES", "10")),
        int(os.getenv("RECOVERY_MAX_AUTOMATIC_ATTEMPTS", "3")),
        backoff,
    )
    if (
