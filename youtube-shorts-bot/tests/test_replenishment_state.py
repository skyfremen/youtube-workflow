...
def decision(provider_id="123", result="approve", attempt=1):
    ...
    return {
        ...
        "attempt": attempt,
        "evidence": {
            "review_index_path": f"youtube-shorts-bot/content/background-sourcing/review-evidence/dr-{SESSION[3:]}-a{attempt:02d}-run-123.json",
            "workflow_run_id": 123,
            "artifact_id": 456,
...
def event(status="PASS", attempt=1):
    ...
    return {
        ...
        "readiness_manifest_path": f"youtube-shorts-bot/content/background-sourcing/readiness/{SESSION}-a{attempt:02d}.json",
        "ingest_summary": {"accepted_count": 1},
        "media_readiness": readiness(0) | {"status": status, "ready": status == "PASS"},
        "continuation_phase": phase,
        "recorded_at": "2026-09-14T03:10:00+08:00",
    }


def test_targeted_request_carries_stable_invocation_deficits_and_exclusions():
    doc = request(excluded=["99", "123", "99"])
    assert doc["schema_version"] == 3
    assert doc["target_categories"] == ["cleaning"]
    assert doc["category_deficits"]["cleaning"] == 1
    assert doc["exclude_provider_asset_ids"] == ["99", "123"]
    assert doc["planner_invocation"] == invocation()
    assert pexels_discovery._load_request_dict(doc) == doc


def test_attempt_specific_query_strategies_materially_change():
    rounds = [pexels_discovery.queries_for_attempt("cleaning", value) for value in range(1, 6)]
    assert len(set(rounds)) == 5
    assert all(rounds[index] != rounds[index + 1] for index in range(4))


def test_rejected_provider_is_excluded_and_session_needs_next_attempt():
    ...
    assert state["continuation_phase"] == "NEED_DISCOVERY"
    assert state["rejected_provider_asset_ids"] == ["123"]
    assert state["excluded_provider_asset_ids"] == ["123", "777"]


def test_undecided_review_evidence_is_resumable_without_new_request():
    state = derive_session_state(
        SESSION,
        discovery_requests=[request()],
        discovery_results=[result()],
    ...
    assert state["continuation_phase"] == "NEEDS_VISUAL_REVIEW"
    assert state["attempt"] == 1


def test_empty_completed_discovery_round_advances_instead_of_stopping():
    empty_result = result()
...
def test_later_execution_finds_unfinished_compatible_session(tmp_path):
    sourcing = tmp_path / "content" / "background-sourcing"
    requests = sourcing / "discovery-requests"
    requests.mkdir(parents=True)
    (requests / "attempt.json").write_text(json.dumps(request()), encoding="utf-8")
    registry = tmp_path / "media-library" / "backgrounds.json"
    registry.parent.mkdir()
    registry.write_text(json.dumps({"schema_version": 3, "assets": []}), encoding="utf-8")
    state = find_compatible_session(tmp_path, invocation())
    assert state["replenishment_session_id"] == SESSION
    assert state["continuation_phase"] == "WAITING_DISCOVERY"


def test_completed_session_resumes_original_planner_and_is_not_restarted():
    state = derive_session_state(
        SESSION,
        discovery_requests=[request()],
        readiness_events=[event("PASS")],
    )
    assert state["continuation_phase"] == "READY_TO_RESUME"
    assert state["resume_original_planner"] is True
    assert state["planner_invocation"] == invocation()


def test_attempt_five_replenish_is_bounded_exhaustion_with_diagnostic():
    ...
    assert state["continuation_phase"] == "EXHAUSTED"
    assert state["diagnostic"]["error_code"] == "E_MEDIA_REPLENISH_EXHAUSTED"
    assert state["diagnostic"]["attempt_count"] == 5


def test_duplicate_immutable_review_decision_collision_fails():
    with pytest.raises(ValueError, match="[Dd]uplicate review decision collision"):
        derive_session_state(
            SESSION,
            discovery_requests=[request()],
            discovery_results=[result()],
...
def test_category_mismatch_cannot_be_approved():
    doc = decision()
    doc["decisions"][0]["reviewed_category"] = "pov_movement"
    doc["decisions"][0]["category_match"] = False
    assert any("approval requires APPROVED and a category match" in item for item in validate_review_decision(doc))


def test_manifest_must_match_immutable_approved_decision():
    approved = decision()
    decision_id = f"rd-{approved['request_id'][3:]}-123"
    candidate = dict(approved["decisions"][0]["approved_candidate"], review_decision_id=decision_id)
    manifest = {
        "schema_version": 2,
    ...
    assert validate_manifest_against_decisions(manifest, [approved]) == []
    manifest["candidates"][0]["reviewed_category"] = "pov_movement"
    assert any("metadata differs" in item for item in validate_manifest_against_decisions(manifest, [approved]))


def test_invalid_readiness_event_attempt_above_maximum_fails_closed():
    doc = event("REPLENISH", 5)
    doc["attempt"] = 6
    assert any("attempt must be 1-5" in item for item in validate_readiness_event(doc))
...
def _ready_asset(number, category):
    return {
        ...
        "renditions": [{
            "id": str(9000 + number), "width": 1080, "height": 1920,
            "fps": 30.0, "file_type": "video/mp4",
            "direct_url": f"https://videos.pexels.com/existing-{number}.mp4",
        }],
    }


def test_acceptance_reject_then_approve_ingest_pass_and_resume_same_adhoc(tmp_path, monkeypatch):
    categories = []
    for category, minimum in REQUIRED_CATEGORY_MINIMUMS.items():
        categories.extend([category] * (minimum - (1 if category == "cleaning" else 0)))
    categories.extend(["satisfying_process"] * (MIN_SELECTABLE_ASSETS - 1 - len(categories)))
    registry = {"schema_version": 3, "assets": [
        _ready_asset(index, category) for index, category in enumerate(categories, 1)
    ]}
    ...
    assert state1["continuation_phase"] == "NEED_DISCOVERY"
    request2 = build_next_discovery_request(
        readiness(), SESSION, invocation(), 2, state1["excluded_provider_asset_ids"]
    )
    assert "123" in request2["exclude_provider_asset_ids"]
    assert pexels_discovery.queries_for_attempt("cleaning", 1) != pexels_discovery.queries_for_attempt("cleaning", 2)

    decision_id = f"rd-{approve['request_id'][3:]}-456"
    candidate = dict(approve["decisions"][0]["approved_candidate"], review_decision_id=decision_id)
    manifest = {
        "schema_version": 2,
        "plan_date": "2026-09-14",
        "provider": "Pexels",
    ...
    (decisions_root / "approve.json").write_text(json.dumps(approve), encoding="utf-8")
    video = {
        "id": 456, "width": 1080, "height": 1920, "duration": 90,
        "url": "https://www.pexels.com/video/example-456/", "user": {"name": "Tester"},
        "video_files": [{"id": 4560, "width": 1080, "height": 1920, "fps": 30,
                         "file_type": "video/mp4", "quality": "hd",
                         "link": "https://videos.pexels.com/456.mp4"}],
    }
    monkeypatch.setattr(pexels_registry, "api_get", lambda *args, **kwargs: video)
    ingest = pexels_resilient_ingest.ingest_resilient(
        manifest_path, registry_path, review_decisions_root=tmp_path / "decisions"
    )
    assert ingest["accepted_count"] == 1
    persisted = json.loads(registry_path.read_text(encoding="utf-8"))
    event_manifest = dict(manifest, readiness_manifest_path=str(manifest_path))
    readiness_event = build_readiness_event(
        event_manifest, ingest, persisted, recorded_at="2026-09-14T03:10:00+08:00"
    )
    assert readiness_event["media_readiness"]["status"] == "PASS"
    ...
    assert state2["continuation_phase"] == "READY_TO_RESUME"
    assert state2["resume_original_planner"] is True
    assert state2["planner_invocation"] == invocation()
