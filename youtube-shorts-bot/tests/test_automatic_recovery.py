...
class DecisionTests(unittest.TestCase):
    ...
    def test_failed_private_dispatch_bypasses_grace(self):
        result = recovery.decide(
            snapshot(forced_source_failure=True,
    ...
    def test_retryable_failure_recovers_immediately_on_first_attempt(self):
        result = recovery.decide(
            snapshot(latest_event='diagnostic', latest_event_at=NOW - timedelta(minutes=1),
                     latest_retryable=True, latest_error_code='E_EXEC_001'),
            NOW, self.policy,
        )
        self.assertTrue(result.dispatch)
    ...
    def test_nonretryable_failure_is_terminal(self):
        result = recovery.decide(
            snapshot(latest_event='diagnostic', latest_event_at=NOW,
                     latest_retryable=False), NOW, self.policy,
        )
        self.assertTrue(result.terminal)
    ...
    def test_unknown_retryability_is_terminal(self):
        result = recovery.decide(
            snapshot(latest_event='diagnostic', latest_event_at=NOW,
                     latest_retryable=None), NOW, self.policy,
        )
        self.assertTrue(result.terminal)
    ...
    def test_second_attempt_obeys_backoff(self):
        result = recovery.decide(
            snapshot(automatic_attempts=1, latest_event='diagnostic',
                     latest_event_at=NOW - timedelta(minutes=30), latest_retryable=True),
            NOW, self.policy,
        )
    ...
    def test_second_attempt_runs_after_backoff(self):
        result = recovery.decide(
            snapshot(automatic_attempts=1, latest_event='diagnostic',
                     latest_event_at=NOW - timedelta(minutes=121), latest_retryable=True),
            NOW, self.policy,
        )
        self.assertTrue(result.dispatch)
    ...
    def test_recovery_in_progress_is_active(self):
        result = recovery.decide(
            snapshot(automatic_attempts=1, latest_batch_id='r_' + 'b' * 30,
    ...
    def test_closed_slot_without_durable_evidence_is_terminal(self):
        ...
        self.assertTrue(result.terminal)
        self.assertEqual(result.reason, 'scheduled_window_closed_without_upload_evidence')

    def test_completed_batch_without_receipt_is_recoverable(self):
        result = recovery.decide(
            snapshot(latest_event='completion', latest_event_at=NOW - timedelta(minutes=1)),
            NOW, self.policy,
        )
        self.assertTrue(result.dispatch)
        self.assertEqual(result.reason, 'batch_completed_without_authoritative_receipt')

    def test_retryable_verification_failure_is_recoverable(self):
        result = recovery.decide(
            snapshot(latest_event='diagnostic', latest_event_at=NOW - timedelta(minutes=1),
                     latest_retryable=True, latest_error_code='E_VERIFY_001', latest_stage='verify'),
            NOW, self.policy,
        )
        self.assertTrue(result.dispatch)
...
class IdentityTests(unittest.TestCase):
    def test_duplicate_watchdog_invocations_resolve_same_batch(self):
        items = [
            {'content_id': 'wd-a', 'source_commit_sha': 'a' * 40, 'automatic_attempt': 1},
            {'content_id': 'wd-b', 'source_commit_sha': 'b' * 40, 'automatic_attempt': 2},
        ]
        self.assertEqual(
    ...
    def test_attempt_number_changes_recovery_identity(self):
        one = [{'content_id': 'wd-a', 'source_commit_sha': 'a' * 40, 'automatic_attempt': 1}]
        two = [{'content_id': 'wd-a', 'source_commit_sha': 'a' * 40, 'automatic_attempt': 2}]
        self.assertNotEqual(recovery.automatic_batch_id(one), recovery.automatic_batch_id(two))
...
class ReconciliationTests(unittest.TestCase):
    def test_23_of_24_success_recovers_only_missing_item(self):
        ...
        for index in range(24):
            ...
            snapshots[content_id] = snapshot(
                content_id=content_id, receipt_state='valid' if index < 23 else 'missing'
            )
        with tempfile.TemporaryDirectory() as tmp:
            repo = FakeRepo(tmp, snapshots)
            result = recovery.reconcile(repo, now=NOW, policy=recovery.Policy(), write=True)
    ...
    def test_24_of_24_success_dispatches_nothing(self):
        snapshots = {
            f'wd-item-{index:02d}': snapshot(content_id=f'wd-item-{index:02d}', receipt_state='valid')
            for index in range(24)
        }
        with tempfile.TemporaryDirectory() as tmp:
            repo = FakeRepo(tmp, snapshots)
