...
class AnalyticsFreshStartTests(unittest.TestCase):
    ...
    def test_net_subscriber_rate_uses_gained_minus_lost(self):
        ...
        row = {
            "video": "abcdefghijk", "views": 1000, "engagedViews": 600,
            "averageViewDuration": 90, "averageViewPercentage": 70,
    ...
    def test_model_uses_age_matched_cohort_and_scores_candidate_attributes(self):
        videos = []
        milestone_videos = {}
        for i in range(ANALYTICS_MIN_MATURE_VIDEOS):
            vid = f"video-{i}"
            high = i >= ANALYTICS_MIN_MATURE_VIDEOS // 2
            ...
            milestone_videos[vid] = {"24h": {"age_hours": 25, "metrics": {
        ...
        components = {
            "curiosity_gap": 90, "emotional_impact": 90, "unanswered_question": 90,
            "immediate_comprehension": 90, "specificity": 90, "natural_phrasing": 90,
            "conciseness": 90, "truthful_reflection": 90,
        }
        strong = score_candidate({
            ...
            "title_candidates": [{
                "title": "Truth #Shorts", "style": "HIDDEN_REVELATION",
                "truthful": True, "score_components": components,
            }],
        }, model)
        weak = score_candidate({
            ...
            "title_candidates": [{
                "title": "Office #Shorts", "style": "DISCOVERY",
                "truthful": True, "score_components": components,
            }],
        }, model)
        self.assertGreater(strong["historical_attribute_fit"], weak["historical_attribute_fit"])
