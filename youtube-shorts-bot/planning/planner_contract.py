...
def build_contract():
 ...
 readiness.update({
  ...
  "replenishment_state_implementation":"media.replenishment_state","review_decision_prefix":"content/background-sourcing/review-decisions/","readiness_event_prefix":"content/background-sourcing/replenishment-events/","discovery_request_schema_version":3,"review_decision_schema_version":2,"readiness_manifest_schema_version":2,"readiness_event_schema_version":1,
  "targeted_deficit_discovery":True,"target_only_current_deficits":True,"rejected_candidates_are_excluded_from_retry":True,"exclude_previously_reviewed_provider_assets":True,"resume_same_planner_invocation":True,"preserve_original_planner_invocation":True,
  ...
  "replenishment_sequence":["audit returns REPLENISH","resume or create one stable replenishment session for the current planner invocation","create a schema-v3 targeted discovery request for current deficits excluding active and all same-session discovered/reviewed provider IDs","run Background Management for deterministic discovery and visual-evidence transport","ChatGPT reviews exact-source evidence and persists immutable evidence-bound approve/reject decisions","ingest only approved category-matching candidates, rerun media.media_readiness, and persist a post-ingestion readiness event","if still REPLENISH increment the same session attempt and repeat, up to 5 attempts","on PASS resume Daily or Ad-hoc ranked-pool authorship in the original planner invocation"],
